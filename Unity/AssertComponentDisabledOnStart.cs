...
{
    /// <summary>Verifies that a gameobject is disabled when this gameobject is instantiated or turned on.</summary>
        ...
        public void Start()
        {
            if (TargetObject.activeInHierarchy)
            {
                throw new ArgumentException("Gameobject [" + TargetObject.name +
                                            "] should be disabled when the gameobject [" + gameObject.name +
                                            "] is turned on.");
            }
        }
    }
